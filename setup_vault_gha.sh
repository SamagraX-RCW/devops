...
check_vault_status() {
  vault_status=$(docker  exec -it $SERVICE_NAME /bin/sh -c "export VAULT_ADDR=http://0.0.0.0:8200 && vault status 2>&1")
...
if [[ $vault_status == *"Initialized     true"* ]]; then
  ...
  docker exec -it $SERVICE_NAME /bin/sh -c "export VAULT_ADDR=http://127.0.0.1:8200 && vault operator init" > ansi-keys.txt
...
docker exec -it $SERVICE_NAME /bin/sh -c "export VAULT_ADDR=http://127.0.0.1:8200 && vault operator unseal "$key" < /dev/null"
